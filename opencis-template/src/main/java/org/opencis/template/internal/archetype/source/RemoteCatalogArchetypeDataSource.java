...
public class RemoteCatalogArchetypeDataSource extends
	...
	private static final Logger logger = LoggerFactory
			.getLogger(RemoteCatalogArchetypeDataSource.class);
	 ...
	 * authentication info can be setup in settings.xml.
	 */
	 ...
	 * @param wagonManager
	 */
	public RemoteCatalogArchetypeDataSource(WagonManager wagonManager) {
	...
	public ArchetypeCatalog getArchetypeCatalog(Properties properties)
		...
		if (repository == null) {
			throw new ArchetypeDataSourceException(
					"To use the remote catalog you must specify the 'repository' property with an URL.");
		}
		...
		try {
			...
			throw e;
		} catch (Exception e) { // When the default archetype catalog name
								// doesn't work, we assume the repository is the
								// URL to a file
			String repositoryPath = repository.substring(0,
					repository.lastIndexOf("/"));
			String filename = repository
					.substring(repository.lastIndexOf("/") + 1);

			try {
				return downloadCatalog(repositoryPath, filename);
			} catch (Exception ex) {
				logger.warn("Error reading archetype catalog " + repository, ex);
	...
	private ArchetypeCatalog downloadCatalog(String repositoryPath,
			String filename) throws WagonException, IOException,
			ArchetypeDataSourceException {
		logger.debug("Searching for remote catalog: " + repositoryPath + "/"
				+ filename);

		// We use wagon to take advantage of a Proxy that has already been setup
		// in a Maven environment.
		Repository wagonRepository = new Repository(REPOSITORY_ID,
				repositoryPath);
		AuthenticationInfo authInfo = wagonManager
				.getAuthenticationInfo(wagonRepository.getId());
		ProxyInfo proxyInfo = wagonManager.getProxy(wagonRepository
				.getProtocol());
	...
	private void disconnectWagon(Wagon wagon) {
		try {
			...
			logger.warn("Problem disconnecting from wagon - ignoring: "
					+ e.getMessage());
		}
	}
