 ...
 * 
 */
package org.opencis.template.internal.artifact.manager;
...
public class DefaultWagonManager implements WagonManager, Contextualizable {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(DefaultWagonManager.class);
	...
	public void putArtifactMetadata(File source,
			ArtifactMetadata artifactMetadata, ArtifactRepository repository)
			throws TransferFailedException {
		logger.info("Uploading " + artifactMetadata);
		putRemoteFile(repository, source,
				repository.pathOfRemoteRepositoryMetadata(artifactMetadata),
				null);
	}

	private void putRemoteFile(ArtifactRepository repository, File source,
			String remotePath, TransferListener downloadMonitor)
			throws TransferFailedException {
		failIfNotOnline();
		...
		try {
			...
			throw new TransferFailedException(
					"Unable to add checksum methods: " + e.getMessage(), e);
		}
		...
		try {
			...
			if (serverPermissionsMap.containsKey(repository.getId())) {
				RepositoryPermissions perms = (RepositoryPermissions) serverPermissionsMap
						.get(repository.getId());

				logger.debug("adding permissions to wagon connection: "
				...
				if (defaultRepositoryPermissions != null) {
					artifactRepository
							.setPermissions(defaultRepositoryPermissions);
				} else {
					logger.debug("not adding permissions to wagon connection");
				}
			}
			...
			for (Iterator i = checksums.keySet().iterator(); i.hasNext();) {
				...
				ChecksumObserver observer = (ChecksumObserver) checksums
						.get(extension);
				sums.put(extension, observer.getActualChecksum());
			}

			// We do this in here so we can checksum the artifact metadata too,
			// otherwise it could be metadata itself
			for (Iterator i = checksums.keySet().iterator(); i.hasNext();) {
				String extension = (String) i.next();
			...
			throw new TransferFailedException("Authentication failed: "
					+ e.getMessage(), e);
			...
			throw new TransferFailedException(
					"Error creating temporary file for deployment: "
							+ e.getMessage(), e);
		} finally {
	...
	public void getArtifact(Artifact artifact, List remoteRepositories)
		...
		for (Iterator iter = remoteRepositories.iterator(); iter.hasNext()
				&& !successful;) {
		...
		// if it already exists locally we were just trying to force it - ignore
		// the update
	...
	public void getArtifact(Artifact artifact, ArtifactRepository repository)
		...
		ArtifactRepositoryPolicy policy = artifact.isSnapshot() ? repository
				.getSnapshots() : repository.getReleases();

		if (!policy.isEnabled()) {
			logger.debug("Skipping disabled repository " + repository.getId());
		} else if (repository.isBlacklisted()) {
			logger.debug("Skipping blacklisted repository "
					+ repository.getId());
		} else {
	...
	private void getRemoteFile(ArtifactRepository repository, File destination,
			String remotePath, TransferListener downloadMonitor,
		...
		if (mirror != null) {
			repository = repositoryFactory.createArtifactRepository(
					mirror.getId(), mirror.getUrl(), repository.getLayout(),
		...
		try {
			...
			// this will run at most twice. The first time, the firstRun flag is
			// turned off, and if the retry flag
			// is set on the first run, it will be turned off and not re-set on
			// the second try. This is because the
			...
			while (firstRun || retry) {
				...
				if (destination.exists() && !force) {
					try {
						...
						if (!downloaded) {
							// prevent additional checks of this artifact until
							// it expires again
							destination.setLastModified(System
									.currentTimeMillis());
						}
				...
				if (downloaded) {
					...
					try {
						verifyChecksum(sha1ChecksumObserver, destination, temp,
								remotePath, ".sha1", wagon);
						...
						if (firstRun) {
							...
							handleChecksumFailure(checksumPolicy,
									e.getMessage(), e.getCause());
						}
					} catch (ResourceDoesNotExistException sha1TryException) {
						logger.debug("SHA1 not found, trying MD5",
								sha1TryException);
	...
	private void handleChecksumFailure(String checksumPolicy, String message,
			Throwable cause) throws ChecksumFailedException {
		if (ArtifactRepositoryPolicy.CHECKSUM_POLICY_FAIL
				.equals(checksumPolicy)) {
			throw new ChecksumFailedException(message, cause);
		} else if (!ArtifactRepositoryPolicy.CHECKSUM_POLICY_IGNORE
				.equals(checksumPolicy)) {
	...
	private void verifyChecksum(ChecksumObserver checksumObserver,
			File destination, File tempDestination, String remotePath,
	...
	private void disconnectWagon(Wagon wagon) {
		try {
			...
			logger.error("Problem disconnecting from wagon - ignoring: "
					+ e.getMessage());
		}
	}
	...
	private void releaseWagon(String protocol, Wagon wagon) {
		...
		try {
			...
			logger.error("Problem releasing wagon - ignoring: "
					+ e.getMessage());
		}
	}
	...
	public ArtifactRepository getMirror(String mirrorOf) {
		ArtifactRepository repository = (ArtifactRepository) mirrors
				.get(mirrorOf);
		if (repository == null) {
	 ...
	 */
	public void addProxy(String protocol, String host, int port,
	...
	public void addPermissionInfo(String repositoryId, String filePermissions,
	 ...
	 * 
	 * @param wagon
	 *            the wagon to configure
	 * @param repository
	 *            the repository that has the configuration
	 * @throws WagonConfigurationException
	 *             wraps any error given during configuration of the wagon
	 *             instance
	 */
			...
			try {
				...
				if (componentConfigurator != null) {
					try {
						...
						logger.error("Problem releasing configurator - ignoring: "
								+ e.getMessage());
					}
				}
	...
	public void addConfiguration(String repositoryId, Xpp3Dom configuration) {
		...
		final XmlPlexusConfiguration xmlConf = new XmlPlexusConfiguration(
				configuration);

		serverConfigurationMap.put(repositoryId, xmlConf);
	}

	public void setDefaultRepositoryPermissions(
			RepositoryPermissions defaultRepositoryPermissions) {
